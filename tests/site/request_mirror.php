...
if ($wantsHtml) {
    ...
    <body>
        ...
        <h1>Request</h1>
        <dl>
            <dt>Protocol version</dt><dd><?php echo htmlspecialchars($_SERVER['SERVER_PROTOCOL'] ?? ''); ?></dd>
            <dt>Request method</dt><dd><?php echo htmlspecialchars($_SERVER['REQUEST_METHOD'] ?? ''); ?></dd>
            <dt>Accept header</dt><dd><?php echo htmlspecialchars($_SERVER['HTTP_ACCEPT'] ?? ''); ?></dd>
        </dl>
        <h1>Cookies</h1>
        <?php
            if (!empty($_COOKIE)) {
        ...
        ?>
        <h1>Raw GET data</h1>
        <?php if (!empty($_SERVER['QUERY_STRING'])) { echo '[' . htmlspecialchars($_SERVER['QUERY_STRING']) . ']'; } ?>
        <h1>GET data</h1>
        <?php
            $get = PageRequest::get();
            if (count($get) > 0) {
                foreach ($get as $k => $v) {
                    ...
                    echo htmlspecialchars($k) . '=[' . htmlspecialchars((string) $v) . "]<br />\n";
                }
            }
        ?>
            ...
            if (!empty($_POST)) {
                function show_array_value($array)
                {
                    ...
                    foreach ($array as $key => $value) {
                        ...
                        if (is_array($value)) { $html .= show_array_value($value); } else { $html .= htmlspecialchars((string) $value); }
                        $html .= ']';
                    }
                    return $html;
                }
