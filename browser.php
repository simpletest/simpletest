...
class SimpleBrowserHistory {
    ...
    protected function atBeginning() {
        return ($this->_position == 0) && ! $this->isEmpty();
    }
    ...
    protected function atEnd() {
        return ($this->_position + 1 >= count($this->_sequence)) && ! $this->isEmpty();
    }
...
class SimpleBrowser {
     ...
     */
    function __construct() {
        $this->_user_agent = $this->createUserAgent();
    ...
    protected function loadPage($url, $parameters) {
        $this->_page = $this->fetch($url, $parameters);
    ...
    function retry() {
        ...
        if ($url = $this->_history->getUrl()) {
            $this->_page = $this->fetch($url, $this->_history->getParameters());
            return $this->_page->getRaw();
        }
        return false;
    }
